    ...
    $( document ).ready(function() {
		...
		getSparkRooms = "/anypoint/api/integrations/"+integration_id+"/getSparkRooms";
	...
	function setUpRooms(){
		if(allRooms.length !=0){
			...
			if(appName=="UserVoice"){} 
			else {if(!isMobile)$('#rooms').customselect("destroy");}
			$.each(allRooms, function(id,obj){
			...
			selected_room_id = 0;
			if(appName=="UserVoice"){} 
			else { if(!isMobile)$("#rooms").customselect();}
		}
	} 
